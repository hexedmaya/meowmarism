// Local user store for panel login: a small JSON-backed database.
// createUserStore(file) gives each caller its own independent store.
//
// Accounts are "owner" (exactly one, created only by install.sh) or "member".
// What a member may do is not a role but a set of permissions: a few
// panel-wide ones plus instance capabilities, granted as a default for all
// instances and optionally overridden per instance.
const fs = require('fs');
